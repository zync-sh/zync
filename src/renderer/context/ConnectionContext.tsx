...
export function ConnectionProvider({ children }: { children: ReactNode }) {
  ...
  }, [tabs, activeTabId]); // Check when tabs change length (init)
  ...
  useEffect(() => {
    const checkConnections = async () => {
      ...
      for (const c of connections) {
        // Only check ALREADY connected sessions.
        // If it is 'connecting', leave it alone; the connect() function will handle success/fail.
        if (c.status === 'connected') {
          try {
            const isAlive = await window.ipcRenderer.invoke('ssh:status', c.id);
            // console.log(`[ConnectionContext] poll ${c.id} -> ${isAlive}`);
            if (!isAlive) {
              console.warn(`[ConnectionContext] Connection lost for ${c.id} (poll returned ${isAlive})`);
              setConnections((prev) =>
                prev.map((conn) => (conn.id === c.id ? { ...conn, status: 'disconnected' } : conn)),
              );
            }
          } catch (error) {
            console.error(`[ConnectionContext] Poll error for ${c.id}:`, error);
            setConnections((prev) =>
              prev.map((conn) => (conn.id === c.id ? { ...conn, status: 'disconnected' } : conn)),
            );
          }
    ...
    return () => clearInterval(interval);
  }, [connections, showToast]); // Depend on connections to know which to check

  // Listen for Backend SSH Events (Errors/Disconnects)
  useEffect(() => {
    const handleError = (_: any, { id, error }: { id: string; error: string }) => {
        console.error(`[ConnectionContext] Received SSH Error for ${id}:`, error);
        setConnections((prev) => prev.map((c) => (c.id === id ? { ...c, status: 'error' } : c)));
        showToast('error', `Connection Error: ${error}`);
    };

    const handleDisconnect = (_: any, { id, reason }: { id: string; reason: string }) => {
        console.warn(`[ConnectionContext] Received SSH Disconnect for ${id}:`, reason);
        // Only update if currently connected/connecting to avoid spam on manual disconnect
        // But we can't easily check current state inside callback unless we use ref or functional update
        setConnections((prev) => {
             const conn = prev.find(c => c.id === id);
             if (conn && conn.status !== 'disconnected') {
                 showToast('error', `Connection Lost: ${reason}`);
                 return prev.map((c) => (c.id === id ? { ...c, status: 'disconnected' } : c));
             }
             return prev;
        });
    };

    window.ipcRenderer.on('ssh:error', handleError);
    window.ipcRenderer.on('ssh:disconnect', handleDisconnect);

    return () => {
        window.ipcRenderer.removeAllListeners('ssh:error');
        window.ipcRenderer.removeAllListeners('ssh:disconnect');
    };
  }, [showToast]);
  ...
  useEffect(() => {
    if (connections.length > 0) {
      const toSave = connections.map(
        ({ id, name, host, username, port, privateKeyPath, jumpServerId, lastConnectedAt }) => ({
          id,
          name,
          host,
          username,
  ...
  const openAddConnectionModal = () => setIsAddConnectionModalOpen(true);
  const closeAddConnectionModal = () => setIsAddConnectionModalOpen(false);
  ...
  const openTab = async (connectionId: string) => {
    ...
    // Always attempt to connect - the connect() function will check if already connected
  ...
  const connect = async (id: string) => {
    console.log('[ConnectionContext] connect() called for:', id);
    if (id === 'local') {
    ...
    try {
      ...
      console.log('[ConnectionContext] Connection found, current status:', connection.status);

      // Prevent duplicate connection attempts
      if (connection.status === 'connecting' || connection.status === 'connected') {
        console.log('[ConnectionContext] Already connecting/connected, checking if alive...');
        const isAlive = await window.ipcRenderer.invoke('ssh:status', id);
        console.log('[ConnectionContext] isAlive check result:', isAlive);
        if (isAlive) {
      ...
      console.log('[ConnectionContext] Calling ssh:connect IPC...');
      ...
      setConnections((prev) =>
        prev.map((c) => (c.id === id ? { ...c, status: 'connected', lastConnectedAt: Date.now() } : c)),
      );
      showToast('success', `Connected to ${connection.name || connection.host}`);
    } catch (e: any) {
      console.error('[ConnectionContext] Connection error:', e);
      setConnections((prev) => prev.map((c) => (c.id === id ? { ...c, status: 'error' } : c)));
