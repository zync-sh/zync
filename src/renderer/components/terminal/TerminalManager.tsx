    ...
    <div className="flex flex-col h-full bg-app-bg">
      ...
      <div className="flex items-center bg-app-panel border-b border-app-border">
          ...
          {tabs.map((tab) => (
            ...
            >
              ...
              <button
                ...
                className="opacity-0 group-hover:opacity-100 p-0.5 rounded hover:bg-app-text/10 text-app-muted hover:text-app-text transition-all"
              >
                <X size={12} />
              </button>
            </div>
        ...
        <button
          ...
          className="p-2 text-app-muted hover:text-app-text hover:bg-app-surface border-l border-app-border transition-colors h-full aspect-square flex items-center justify-center"
      ...
      <div className="flex-1 overflow-hidden relative bg-app-bg">
        {tabs.length === 0 ? (
          <div className="h-full flex flex-col items-center justify-center text-app-muted">
            ...
            <button onClick={handleNewTab} className="mt-4 text-app-accent hover:underline">
              Open New Terminal
            </button>
          </div>
        ) : (
          tabs.map((tab) => (
            <div key={tab.id} className={cn('absolute inset-0', activeTabId === tab.id ? 'z-10' : 'z-0 invisible')}>
